...
UserSchema.statics.addPoems = function(id, title) {
  const Poem = mongoose.model('poem');
  return this.findById(id)
    .then(user => {
      const poem = new Poem({ title, user })
      user.poem.push(poem)
      return Promise.all([poem.save(), user.save()])
        .then(([poem, user]) => user);
    });
}

UserSchema.statics.findPoems = function(id) {
  return this.findById(id)
    .populate('poem')
    .then(user => user.poem);
}
